<?php
session_start();
require_once __DIR__ . "/../../config/koneksi.php";
require_once __DIR__ . "/helper.php";

// Cek role admin
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
    header("Location: ../../auth/login.php");
    exit;
}

try {
    // Ambil dan sanitize input
    $id = (int)($_POST['id'] ?? 0);
    $gedung_id = sanitizeGedungId($_POST['gedung_id'] ?? 0);
    $nomor = sanitizeNumber($_POST['nomor'] ?? 0);

    if (!$id) {
        throw new Exception("ID lantai tidak valid");
    }

    // Validasi input
    validateLantaiInput($gedung_id, $nomor);

    // Cek gedung ada
    if (!checkGedungExists($gedung_id)) {
        throw new Exception("Gedung tidak ditemukan!");
    }

    // Cek lantai sudah digunakan
    if (checkLantaiExists($gedung_id, $nomor, $id)) {
        throw new Exception("Lantai dengan nomor ini sudah ada di gedung yang sama!");
    }

    // Update lantai
    $sql = "UPDATE lantai SET gedung_id = ?, nomor = ? WHERE id = ?";
    query($sql, [$gedung_id, $nomor, $id]);

    header("Location: ../kelola_lantai.php?success=edit");
    exit;
} catch (Exception $e) {
    header("Location: ../kelola_lantai.php?error=" . urlencode($e->getMessage()));
    exit;
}
