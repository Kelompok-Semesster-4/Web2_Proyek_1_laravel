@php
    function iconFasilitas(string $nama): string
    {
        ...
        foreach ($map as $k => $ico)
            if (str_contains($n, $k))
                return $ico;
        if ($n === 'ac' || str_contains($n, ' ac') || str_contains($n, 'ac '))
            return 'bi-snow';
...
<x-layouts.mahasiswa-layout>
    <x-slot:title>Detail Ruangan - {{ $ruangan->nama_ruangan ?? 'Peminjaman Ruangan' }}</x-slot>
                        ...
                        <div class="row g-4 align-items-start">
                            <div class="col-lg-5">
                                <div class="d-flex flex-wrap gap-2 mb-3">
                                    ...
                                    <span class="info-pill">🏬 Lantai
                                        {{ $ruangan->Lantai ?? ($ruangan->lantai ?? '-') }}</span>
        ...
        <x-modal-admin id="imagePreviewModal" title="Foto Ruangan" modal-class="modal fade img-modal"
            dialog-class="modal-dialog modal-dialog-centered modal-xl" header-gradient="#212529">
