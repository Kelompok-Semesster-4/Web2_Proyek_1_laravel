<x-layouts.admin-layout>
    <x-slot:title>Persetujuan Peminjaman - Admin</x-slot>
        ...
        <div class="admin-container" style="max-width: 100%;">
            <!-- Page Header -->
            <x-head-title-admin title="Persetujuan Peminjaman" icon="bi bi-clipboard-check" :showButton="false" />
            <!-- Alert Messages -->
            ...
            <x-table-card title="Daftar Persetujuan" icon="bi bi-list-ul" table-id="tablePersetujuan"
                ...
                @foreach ($pending as $i => $p)
                    <tr>
                        ...
                        <td class="text-center">
                            @if (!empty($p->surat))
                                <a class="btn btn-warning" href="{{ asset('storage/uploads/surat/' . $p->surat) }}"
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.approve.process') }}"
                                ...
                                <div class="d-flex justify-content-center">
                                    <button type="button" class="btn btn-success aksi-btn me-2 approve-action-btn"
                                        data-action="approve">
                                        <i class="bi bi-check-circle me-1"></i>Setujui
                                    </button>

                                    <button type="button" class="btn btn-danger aksi-btn reject-action-btn"
                                        data-action="reject">
