        ...
        <section class="card shadow-sm border-0 dashboard-panel laporan-filter-card mb-4">
            ...
            <div class="card-body">
                <form class="laporan-filter-form" method="GET" action="{{ route('admin.laporan') }}">
                    <input type="hidden" name="year" value="{{ \Illuminate\Support\Carbon::parse($startDate)->year }}">
                    <input type="hidden" name="month" value="{{ \Illuminate\Support\Carbon::parse($startDate)->month }}">
                    ...
                    <div class="laporan-filter-actions">
                        <span class="laporan-filter-total text-muted fw-semibold">
                            Menampilkan <strong>{{ count($detail) }}</strong> dari <strong>{{ $totalRequests }}</strong> total data
                        </span>
                        <div class="d-flex flex-wrap gap-2">
                            <a href="{{ route('admin.laporan') }}" class="btn btn-outline-secondary laporan-filter-btn">
                                <i class="bi bi-arrow-counterclockwise me-1"></i>Reset Filter
                            </a>
                            <button type="submit" class="btn btn-success laporan-filter-btn">
                                <i class="bi bi-check2-circle me-1"></i>Terapkan Filter
                            </button>
                            <a class="btn btn-success laporan-filter-btn" href="{{ route('admin.laporan', $exportParams) }}">
        ...
        <section class="card shadow-sm border-0 dashboard-panel mb-4">
            <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <small class="text-muted">
                    <i class="bi bi-info-circle me-1"></i>Kapasitas: {{ number_format($capacityHours, 0) }} jam per periode
                </small>
            </div>
            <div class="card-body">
                <div class="row g-3">
                        ...
                        <div class="col-lg-4">
                            <article class="laporan-room-card h-100">
                                ...
                                <div class="laporan-room-metrics">
                                    ...
                                    <div><strong>{{ number_format($room->used_hours, 1) }}</strong><span>Jam</span></div>
                                    <div><strong class="text-success">{{ number_format($room->utilization_rate, 1) }}%</strong><span>Utilitas</span></div>
                                </div>
                                <div class="d-flex justify-content-between text-muted small mt-3">
                                    <span>Utilization Rate</span>
                                    <strong>{{ number_format($room->used_hours, 1) }}/{{ number_format($room->capacity_hours, 0) }} jam</strong>
                                </div>
                                <div class="progress mt-2">
                                    <div class="progress-bar bg-success" style="width: {{ $room->utilization_rate }}%"></div>
                                </div>
                            </article>
                        </div>
                    @empty
        ...
        <x-table-card title="Detail Transaksi" icon="bi bi-table" table-id="tableLaporan"
                ...
                <tr>
                    ...
                    <th style="padding: 15px 10px;">
                        <a href="{{ $sortLink('tanggal') }}" class="text-decoration-none text-dark d-flex align-items-center justify-content-between">
                            Tanggal & Waktu @if (in_array($sortBy, ['tanggal', 'jam'], true)) <i class="bi bi-sort-{{ $sortDir === 'asc' ? 'up' : 'down' }} text-success"></i> @endif
                        </a>
                    </th>
            ...
            @foreach ($detail as $d)
                ...
                <tr>
                    ...
                    <td>
                        <div class="fw-semibold">{{ \Illuminate\Support\Carbon::parse($d->tanggal)->translatedFormat('d M Y') }}</div>
                        <div class="text-muted">{{ \Illuminate\Support\Carbon::parse($d->jam_mulai)->format('H:i') }}-{{ \Illuminate\Support\Carbon::parse($d->jam_selesai)->format('H:i') }}</div>
                    </td>
                    <td class="text-center">
